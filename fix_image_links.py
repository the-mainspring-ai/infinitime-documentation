...
"""
This script fixes broken image links in markdown documentation files by comparing them with a reference
set of correctly formatted files. It processes markdown files in the docs directory and updates image
references to match the correct format found in the reference files.

The script performs the following operations:
1. Compares markdown files between a correct reference directory and the working directory
2. Identifies broken image references (those pointing to image-404.png)
3. Extracts correct image references from the reference files
4. Updates the working files with the correct image paths
5. Maintains a log of images that couldn't be found

Configuration:
- CORRECT_MD_PATH: Directory containing correctly formatted markdown files
- WORKING_MD_PATH: Directory containing files to be fixed
- IMAGE_PATH: Directory containing the actual image files

Features:
- Handles both relative and absolute image paths
- Preserves original file structure
- Provides detailed logging of changes and errors
- Reports missing images for manual review

Usage:
    Run the script: python fix_image_links.py
"""
